...
namespace App\Repositories\API\DriverUser;

use App\Repositories\EloquentAbstractRepository;
use Illuminate\Contracts\Hashing\Hasher as HasherContract;

class EloquentDriverUserRepository extends EloquentAbstractRepository implements DriverUserRepository
{
    public $hasher;
    ...
    public function authenticate($credentials)
    {
        if ($user = $this->findBy('uEmail', '=', $credentials['email'])->first()) {
            if ($this->hasher->check($credentials['password'], $user->uPassword) && $user->role_id == env('DRIVER_ROLE')) {
                return $user;
            }
        }
