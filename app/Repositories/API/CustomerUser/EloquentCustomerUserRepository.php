...
class EloquentCustomerUserRepository extends EloquentAbstractRepository implements CustomerUserRepository
{
    public $hasher;
    ...
    public function authenticate($credentials)
    {
        if ($user = $this->findBy('uEmail', '=', $credentials['email'])->first()) {
            if ($this->hasher->check($credentials['password'], $user->uPassword) && $user->role_id == env('CUSTOMER_ROLE')) {
                return $user;
            }
        }
